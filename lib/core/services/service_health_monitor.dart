...
import 'persistent_status_notification_service.dart';
...
class ServiceHealthMonitor {
  ...
  static const String _healthChannelDescription = 'Alertes en cas de dysfonctionnement des services';
  ...
  Future<void> _initializeNotifications() async {
    ...
    const androidChannel = AndroidNotificationChannel(
      ...
      importance: Importance.high, // Importance élevée pour les alertes de santé
  ...
  Future<void> _loadSettings() async {
    ...
    if (failuresJson != null) {
      ...
      _serviceFailureCount['geolocation'] = prefs.getInt('failures_geolocation') ?? 0;
      _serviceFailureCount['geofencing'] = prefs.getInt('failures_geofencing') ?? 0;
      _serviceFailureCount['background_location'] = prefs.getInt('failures_background_location') ?? 0;
    }
  }
  ...
  void recordServiceActivity(String serviceName) {
    ...
    if (_serviceFailureCount[serviceName] != null && _serviceFailureCount[serviceName]! > 0) {
  ...
  void recordServiceFailure(String serviceName, String error) {
    _serviceFailureCount[serviceName] = (_serviceFailureCount[serviceName] ?? 0) + 1;
    _saveFailureCount(serviceName, _serviceFailureCount[serviceName]!);
    
    log('ServiceHealthMonitor: Service $serviceName failed: $error (failures: ${_serviceFailureCount[serviceName]})');
  ...
  Future<void> _performHealthCheck() async {
    ...
    for (final serviceName in ['geolocation', 'geofencing', 'background_location']) {
      ...
      if (lastActivity != null) {
        ...
        if (timeSinceActivity > _serviceTimeoutThreshold) {
          _currentIssues.add(ServiceHealthIssue(
            serviceName: serviceName,
            type: HealthIssueType.timeout,
            description: 'Service inactif depuis ${_formatDuration(timeSinceActivity)}',
      ...
      if (failureCount >= 3) {
        _currentIssues.add(ServiceHealthIssue(
          ...
          severity: failureCount >= 5 ? HealthIssueSeverity.critical : HealthIssueSeverity.warning,
          timestamp: now,
        ));
      }
    }
  ...
  Future<void> _sendHealthAlert() async {
    final criticalIssues = _currentIssues.where((i) => i.severity == HealthIssueSeverity.critical).toList();
    final warningIssues = _currentIssues.where((i) => i.severity == HealthIssueSeverity.warning).toList();
    ...
    if (criticalIssues.isNotEmpty) {
      ...
      body = 'Services affectés: ${criticalIssues.map((i) => _getServiceDisplayName(i.serviceName)).join(', ')}';
      notificationColor = Colors.red;
    } else {
      title = '⚠️ Avertissement de service';
      body = 'Services affectés: ${warningIssues.map((i) => _getServiceDisplayName(i.serviceName)).join(', ')}';
  ...
  String _buildDetailedHealthReport() {
    ...
    for (final issue in _currentIssues) {
      ...
      lines.add('$icon ${_getServiceDisplayName(issue.serviceName)}: ${issue.description}');
    }
    
    lines.add('');
    lines.add('Vérifiez les paramètres de l\'application pour plus de détails.');
  ...
  Future<void> setHealthMonitoringEnabled(bool enabled) async {
    ...
    log('ServiceHealthMonitor: Health monitoring ${enabled ? 'enabled' : 'disabled'}');
  }
  ...
  List<ServiceHealthIssue> get currentIssues => List.unmodifiable(_currentIssues);
  ...
  Map<String, dynamic> getHealthStatistics() {
    return {
      ...
      'last_activity': _lastServiceActivity.map((k, v) => MapEntry(k, v.toIso8601String())),
    };
  }
